  ...
  return (
    <div className="flex flex-col pb-24 px-4 pt-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 no-print">
        <div>
          <h1 className="text-xl font-extrabold text-on-surface">Print Assistant</h1>
        ...
        <button
          ...
          className="bg-primary hover:bg-primary/95 text-on-primary disabled:opacity-40 disabled:cursor-not-allowed font-bold text-xs px-4 py-2.5 rounded-xl flex items-center gap-1.5 shadow-sm active:scale-95 transition-all cursor-pointer"
        >
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-xs flex flex-col gap-6 print-container" id="printable-area">
        ...
        <div className="flex justify-between items-start border-b border-dashed border-neutral-200 pb-4">
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-primary text-lg font-black tracking-tighter uppercase">⚡ VELOCITY GRID</span>
            </div>
            <p className="text-[10px] text-outline uppercase font-extrabold tracking-wider mt-0.5">Instant Grocery Delivery</p>
          </div>
        ...
        <div>
          <h3 className="text-xs font-extrabold text-on-surface uppercase tracking-wider mb-2 flex items-center gap-1.5">
              ...
              {cartItems.map((item) => (
                <div key={item.product.id} className="p-3 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2.5">
                    ...
                    />
                    <div>
                      <h4 className="font-bold text-on-surface leading-tight">{item.product.name}</h4>
              ...
              {customItems.map((item) => (
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      <span className={`font-bold text-on-surface ${item.checked ? 'line-through opacity-60' : ''}`}>
                        {item.name}
                      </span>
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-dashed border-neutral-200 pt-4 flex flex-col gap-2 font-mono text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-on-surface">₹{handlingFee}</span>
            </div>
            <div className="flex justify-between">
